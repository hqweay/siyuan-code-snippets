...
export default class PluginGo extends Plugin {
  ...
  //App 准备好时加载
  async onLayoutReady() {
    const result = await this.getCodeSnippets();
    console.log("111");
    console.log(result);
    //这里注入CSS和JS
  ...
  async getCodeSnippets() {
    ...
    // }
    return fetch(
      "https://api.github.com/repos/hqweay/siyuan-code-snippets/issues/1/comments?per_page=100&page=0",
      {
        method: "GET",
        headers: {
              ...
              this.codeSnippets = snippets.map((ele) => {
                ...
                if (body.startsWith("```")) {
                  ...
                  return {
                    id: ele.node_id,
                    type: metaInfos[1],
                    category: metaInfos[2],
                    title: metaInfos[3],
                    author: metaInfos[4],
                    description: metaInfos[5],
                    link: metaInfos[6],
                    code: body.split("\r\n").slice(1, -1).join("\r\n"),
                  };
                }
    ...
    //       return {
    //         type: metaInfos[0],
    //         category: metaInfos[1],
    //         author: metaInfos[2],
    //         description: metaInfos[3],
    //         code: body,
    //       };
    //     }
    //   });
